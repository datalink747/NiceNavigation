...
public class NavigationView extends RelativeLayout {
    ...
    private OnNavigationItemClickListener onBottomNavigationItemClickListener;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < bottomNavigationItems.size(); i++) {
            ...
            TextView title = (TextView) view.findViewById(R.id.navigation_item_title);
            if (isTablet)
    ...
    private void onBottomNavigationItemClick(final int itemIndex) {
        ...
        for (int i = 0; i < viewList.size(); i++) {
            if (i == itemIndex) {
                ...
                final TextView title = (TextView) view.findViewById(R.id.navigation_item_title);
                final ImageView icon = (ImageView) view.findViewById(R.id.navigation_item_icon);
                NavigationUtils.changeTextColor(title, itemInactiveColor, itemActiveColorWithoutColoredBackground);
                NavigationUtils.changeTextSize(title, withText ? textInactiveSize : 0, textActiveSize);
                NavigationUtils.imageColorChange(icon, itemInactiveColor, itemActiveColorWithoutColoredBackground);
                if (isTablet)
                    NavigationUtils.changeRightPadding(view, withText ? viewInactivePaddingTop : viewInactivePaddingTopWithoutText, viewActivePaddingTop);
                else
                    NavigationUtils.changeTopPadding(view, withText ? viewInactivePaddingTop : viewInactivePaddingTopWithoutText, viewActivePaddingTop);
                icon.animate()
                        .setDuration(150)
                        .scaleX((float) 1.1)
                        .scaleY((float) 1.1)
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    NavigationUtils.backgroundColorChange
                ...
                NavigationUtils.changeTextSize(title, textActiveSize, withText ? textInactiveSize : 0);
                if (isTablet)
                    NavigationUtils.changeRightPadding(view, viewActivePaddingTop, withText ? viewInactivePaddingTop : viewInactivePaddingTopWithoutText);
                else
                    NavigationUtils.changeTopPadding(view, viewActivePaddingTop, withText ? viewInactivePaddingTop : viewInactivePaddingTopWithoutText);
                icon.animate()
                        .setDuration(150)
                        .scaleX(1)
                        .scaleY(1)
    ...
    public void setUpWithViewPager(ViewPager pager, int[] colorResources, int[] imageResources, Context ct) {
       // paperflower = PaperFlower.attach2Window(ct);
        ...
        for (int i = 0; i < pager.getAdapter().getCount(); i++)
            addTab(new NavigationItem(pager.getAdapter().getPageTitle(i).toString(), colorResources[i], imageResources[i]));
     ...
     */
    public void addTab(NavigationItem item) {
     ...
     */
    public void setOnBottomNavigationItemClickListener(OnNavigationItemClickListener onBottomNavigationItemClickListener) {
